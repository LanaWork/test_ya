...
class Gallery {
    constructor(e, t={}) {
        ...
        this.size = e.childElementCount,
        this.currentSlide = 0,
        this.currentSlideWasChanged = !1,
        this.settings = {
            margin: t.margin || 0
        },
    ...
    manageHTML() {
        ...
        this.slideNodes = Array.from(this.lineNode.children).map(e=>wrapElementByDiv({
            element: e,
            className: GallerySlideClassName
        })),
        this.dotsNode.innerHTML = Array.from(Array(this.size).keys()).map(e=>`<button class="${GalleryDotClassName} ${e === this.currentSlide ? GalleryDotActiveClassName : ""}"></button>`).join(""),
    ...
    setParameters() {
        ...
        Array.from(this.slideNodes).forEach(e=>{
            e.style.width = `${this.width}px`,
            e.style.marginRight = `${this.settings.margin}px`
        }
        )
    }
    setEvents() {
    ...
    dragging(e) {
        ...
        var t = this.dragX - this.clickX
          , e = t / 5;
        this.x = Math.max(Math.min(this.startX + t, e), this.maximumX + e),
        this.setStylePosition(),
        20 < t && !this.currentSlideWasChanged && 0 < this.currentSlide && (this.currentSlideWasChanged = !0,
        this.currentSlide = this.currentSlide - 1),
        t < -20 && !this.currentSlideWasChanged && this.currentSlide < this.size - 1 && (this.currentSlideWasChanged = !0,
        this.currentSlide = this.currentSlide + 1)
    }
    clickDots(e) {
        ...
        if (s) {
            ...
            for (let e = 0; e < this.dotNodes.length; e++)
                if (this.dotNodes[e] === s) {
                    t = e;
                    break
                }
            t !== this.currentSlide && (e = Math.abs(this.currentSlide - t),
            this.currentSlide = t,
            this.changeCurrentSlide(e))
        }
    }
    moveToLeft() {
        this.currentSlide <= 0 || (this.currentSlide = this.currentSlide - 1,
        this.changeCurrentSlide())
    }
    moveToRight() {
        this.currentSlide >= this.size - 1 || (this.currentSlide = this.currentSlide + 1,
        this.changeCurrentSlide())
    }
    changeCurrentSlide(e) {
        this.x = -this.currentSlide * (this.width + this.settings.margin),
        this.setStyleTransition(e),
        this.setStylePosition(),
        this.changeActiveDotClass(),
        this.changeDisabledNav()
    }
    changeActiveDotClass() {
        for (let e = 0; e < this.dotNodes.length; e++)
            this.dotNodes[e].classList.remove(GalleryDotActiveClassName);
    ...
    changeDisabledNav() {
        ...
        this.currentSlide >= this.size - 1 ? this.navRight.classList.add(GalleryNavDisabledClassName) : this.navRight.classList.remove(GalleryNavDisabledClassName)
    }
    setStylePosition() {
        this.lineNode.style.transform = `translate3d(${this.x}px, 0, 0)`
    }
    setStyleTransition(e=1) {
        this.lineNode.style.transition = `all ${.25 * e}s ease 0s`
    }
